...
bilinear_run = '2025-05-06_17-08-07'
config = OmegaConf.load(cfg_base_path.replace("RUN", bilinear_run))
model_lr = model_pl(config)
ortho_weights = list(Path(ckpt_base_path.replace("RUN", bilinear_run)).glob('epoch=*.ckpt'))[0]
